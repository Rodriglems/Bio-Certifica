...
interface PropsLocalProducao {
  onSave: (data: Partial<DailyRecord>) => void;
  onNavigate: (screen: Screen) => void;
  onExitDailyRecord: () => void;
}

export function LocalProducao({ onSave, onNavigate, onExitDailyRecord }: PropsLocalProducao) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ location: finalLocation });
    onNavigate("production-destination");
  };
          ...
          <div className="flex items-center justify-between pb-4 border-b-2 border-green-100">
            <div className="flex items-center gap-3">
              <div className="bg-green-100 p-3 rounded-full">
                <MapPin size={28} className="text-green-700" strokeWidth={2.5} />
              </div>
